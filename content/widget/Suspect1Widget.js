define([
    "dojo/_base/declare",
    "dojo/_base/lang",
    "dojo/dom-class",
    "dojo/on",
    "dojo/request/xhr",
    "dijit/form/NumberSpinner",
    "dijit/_WidgetBase",
    "dijit/_TemplatedMixin",
    "dijit/_WidgetsInTemplateMixin",
    "dojo/text!./templates/Suspect1Widget.html"
], function(declare, lang, domClass, on, xhr, NumberSpinner, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, template) {
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
        baseClass: "suspect1Widget",
        templateString: template,
        title: "",
        EBCDIC_TO_ASCII: [
            
        ],
        startup: function() {
            this.inherited(arguments);

            on(this.buttonHint1, "click", lang.hitch(this, function(event) {
                domClass.add(this.hint1, "open");
                domClass.add(this.feedbackHint1, "open");
            }));

            on(this.inputWork1, "input", lang.hitch(this, function(event) {
                const hexString = this.inputWork1.value;

                // Strip whitespace and validate the hex string
                const cleanHex = hexString.replace(/\s+/g, '');

                if (cleanHex.length % 2 !== 0) {
                    throw new Error('Hex string must have an even number of characters');
                }
                if (!/^[0-9A-Fa-f]*$/.test(cleanHex)) {
                    throw new Error('Invalid hex string: contains non-hex characters');
                }
                
                let result = '';
                for (let i = 0; i < cleanHex.length; i += 2) {
                    const byteValue = parseInt(cleanHex.substr(i, 2), 16);
                    const asciiCode = this.EBCDIC_TO_ASCII[byteValue];
                    result += String.fromCharCode(asciiCode);
                }
                
                this.feedback1.innerText = result;
            }));

            on(this.buttonRotate, "click", lang.hitch(this, function(event) {
                if (this.inputWork1.value.length > 0) {
                    const rotateBits = this.myNumberSpinner.value;

                    if (rotateBits > 0) {
                        this.inputWork1.value = this.rotateRight(this.inputWork1.value, rotateBits).toUpperCase();
                    } else if (rotateBits < 0) {
                        this.inputWork1.value = this.rotateLeft(this.inputWork1.value, Math.abs(rotateBits)).toUpperCase();
                    }

                    this.inputWork1.dispatchEvent(new Event("input"));
                }
            }));

            on(this.buttonXOR, "click", lang.hitch(this, function(event) {
                if (this.inputWork1.value.length > 0) {
                    this.inputWork1.value = this.xorHex(this.inputWork1.value, "BEEFBEEFBEEFBEEFBEEF").toUpperCase();

                    this.inputWork1.dispatchEvent(new Event("input"));
                }
            }));

            on(this.buttonNibbles, "click", lang.hitch(this, function(event) {
                if (this.inputWork1.value.length > 0) {
                    this.inputWork1.value = this.swapNibbles(this.inputWork1.value).toUpperCase();

                    this.inputWork1.dispatchEvent(new Event("input"));
                }
            }));

            on(this.buttonFinalInput1, "click", lang.hitch(this, function(event) {
                this.finalFeedback1.innerText = "";

                let answer1 = this.inputAnswer1.value;

                if (answer1.length > 0) {
                    let postData = "answer_suspect_1=" + encodeURIComponent(answer1);

                    xhr.post("process_suspect_answer.json", {
                        handleAs: "json",
                        data: postData,
                        preventCache: true,
                        sync: true
                    }).then(lang.hitch(this, function(data) {
                        if (data && data.correct && data.correct == "true") {
                            console.log("yep");
                        } else {
                            this.finalFeedback1.innerText = "That is not correct";
                        }
                    }), lang.hitch(this, function(err) {
                        this.finalFeedback1.innerText = err;
                    }));
                } else {
                    this.finalFeedback1.innerText = "Give a non blank value";
                }
            }));
        },
        normalizeHex: function(hexStr) {
            let s = hexStr.trim().toLowerCase();
            
            if (s.startsWith('0x')) s = s.slice(2);

            if (!/^[0-9a-f]+$/.test(s)) {
                throw new Error(`Invalid hex string: "${hexStr}"`);
            }

            return s;
        },
        hexToBigInt: function(hexStr) {
            return BigInt('0x' + this.normalizeHex(hexStr));
        },
        bigIntToHex: function(value, hexLength) {
            // Mask to the correct bit width, then pad to the original hex length.
            const bits = BigInt(hexLength * 4);
            const mask = (1n << bits) - 1n;
            const masked = value & mask;

            return masked.toString(16).padStart(hexLength, '0');
        },
        rotateLeft: function(hexStr, shift) {
            const hex = this.normalizeHex(hexStr);
            const bits = BigInt(hex.length * 4);
            const n = ((BigInt(shift) % bits) + bits) % bits; // normalize negative/large shifts
            const value = this.hexToBigInt(hex);
            const mask = (1n << bits) - 1n;
            const rotated = ((value << n) | (value >> (bits - n))) & mask;

            return this.bigIntToHex(rotated, hex.length);
        },
        rotateRight: function(hexStr, shift) {
            const hex = this.normalizeHex(hexStr);
            const bits = BigInt(hex.length * 4);
            const n = ((BigInt(shift) % bits) + bits) % bits;

            return this.rotateLeft(hex, bits - n);
        },
        xorHex: function(hexA, hexB) {
          const a = this.normalizeHex(hexA);
          const b = this.normalizeHex(hexB);
          const len = Math.max(a.length, b.length);
          const result = this.hexToBigInt(a) ^ this.hexToBigInt(b);

          return this.bigIntToHex(result, len);
        },
        swapNibbles: function(hexStr) {
          const hex = this.normalizeHex(hexStr);

          if (hex.length % 2 !== 0) {
            throw new Error('swapNibbles requires an even-length hex string (whole bytes)');
          }
          let result = '';
          for (let i = 0; i < hex.length; i += 2) {
            result += hex[i + 1] + hex[i];
          }
          return result;
        }
    });
});